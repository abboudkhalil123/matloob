...
function Content(){const id=window.location.pathname.split('/').filter(Boolean).pop()||'';const [ticket,setTicket]=useState<SupportTicket|null>(null),[messages,setMessages]=useState<SupportMessage[]>([]),[loading,setLoading]=useState(true),[error,setError]=useState(''),[text,setText]=useState(''),[sending,setSending]=useState(false);
 ...
 return <main dir="rtl" className="min-h-screen bg-slate-50 px-4 py-10"><div className="mx-auto max-w-4xl"><a href="/support" className="text-sm font-black text-slate-500">← العودة إلى الدعم</a><div className="mt-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><div className="flex flex-wrap justify-between gap-4"><div><h1 className="text-2xl font-black">{ticket.subject}</h1><p className="mt-2 text-sm text-slate-500">{category(ticket.category)} · أنشئت {date(ticket.created_at)}</p></div><div className="flex gap-2"><span className="rounded-full bg-slate-100 px-3 py-1.5 text-xs font-black">{status(ticket.status)}</span><span className="rounded-full bg-amber-50 px-3 py-1.5 text-xs font-black text-amber-700">{priority(ticket.priority)}</span></div></div><div className="mt-4 text-xs font-bold text-slate-400">آخر تحديث: {date(ticket.updated_at)}</div></div>{error&&<div className="mt-5 rounded-xl bg-rose-50 px-4 py-3 text-sm font-bold text-rose-700">{error}</div>}<div className="mt-6 space-y-3">{messages.map(m=><article key={m.id} className={`rounded-2xl border p-5 ${m.sender_id===ticket.user_id?'border-blue-100 bg-blue-50/50':'border-slate-200 bg-white'}`}><div className="flex justify-between gap-3"><span className="text-sm font-black">{m.sender_id===ticket.user_id?'أنت':'الدعم'}</span><time className="text-xs font-bold text-slate-400">{date(m.created_at)}</time></div><p className="mt-3 whitespace-pre-wrap text-sm leading-8 text-slate-700">{m.message}</p></article>)}</div>{canReply?<div className="mt-6 rounded-3xl border border-slate-200 bg-white p-5"><textarea maxLength={5000} value={text} onChange={e=>setText(e.target.value)} rows={5} placeholder="اكتب ردك..." className="w-full rounded-xl border border-slate-200 p-4 outline-none"/><div className="mt-3 flex items-center justify-between"><span className="text-xs font-bold text-slate-400">{text.length}/5000</span><button disabled={sending||!text.trim()} onClick={()=>void send()} className="rounded-xl bg-slate-950 px-5 py-3 font-black text-white disabled:opacity-40">{sending?'جارٍ الإرسال...':'إرسال الرد'}</button></div></div>:<div className="mt-6 rounded-2xl border border-amber-200 bg-amber-50 p-5"><p className="text-sm font-bold text-amber-900">هذه التذكرة {ticket.status==='closed'?'مغلقة':'محلولة'} ولا يمكن إضافة رد حاليًا.</p><button onClick={()=>void reopen()} className="mt-3 rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-black text-white">إعادة فتح التذكرة</button></div>}</div></main>}
